...
import pt.unl.fct.di.tsantos.util.download.MovieDescriptor;
import pt.unl.fct.di.tsantos.util.download.subtitile.
        OpenSubtitlesSubtitleDescriptor.Property;
...
public class OpenSubtitlesXmlRpc {
    ...
    /**
     * This will login user. This method should be called
     * always when starting talking with server.
     *
     * @param username username (blank for anonymous user)
     * @param password password (blank for anonymous user)
     * @param language ISO639 2-letter codes as language and later
     * communication will be done in this language if
     *            applicable (error codes and so on).
     */
    public synchronized void login(String username,
            String password, String language) throws XmlRpcFault {
        Map<?, ?> response =
                invoke("LogIn", username, password, language, useragent);
     ...
     * Call this function is before closing the client program.
     */
    public synchronized void logout() throws XmlRpcFault {
        try {
            invoke("LogOut", token);
        } catch (XmlRpcFault e) {
            // anonymous users will always get an 401
            // Unauthorized when trying to logout,
    ...
    public List<OpenSubtitlesSubtitleDescriptor> searchSubtitles(int imdbid,
            String... sublanguageids) throws XmlRpcFault {
        return searchSubtitles(singleton(
                Query.forImdbId(imdbid, sublanguageids)));
    }

    @SuppressWarnings("unchecked")
    public List<OpenSubtitlesSubtitleDescriptor> searchSubtitles(
            Collection<Query> queryList) throws XmlRpcFault {
        ...
        try {
            ...
            for (Map<String, String> propertyMap : subtitleData) {
                subtitles.add(new OpenSubtitlesSubtitleDescriptor(
                        Property.asEnumMap(propertyMap)));
            }
    ...
    @SuppressWarnings("unchecked")
    public List<MovieDescriptor> searchMoviesOnIMDB(String query)
            throws XmlRpcFault {
        ...
        for (Map<String, String> movie : movieData) {
            // get non-aka title (aka titles were separated
            // by Â, and then aka later on)
            Scanner titleScanner = new Scanner(
                    movie.get("title")).useDelimiter("(\u00C2)|(\\s+aka\\s+)");

            movies.add(new MovieDescriptor(titleScanner.next().trim(),
    ...
    @SuppressWarnings("unchecked")
    public TryUploadResponse tryUploadSubtitles(SubFile... subtitles)
            throws XmlRpcFault {
        ...
        Map<String, String> subtitleData =
                (Map<String, String>) response.get("data");

        return new TryUploadResponse(uploadRequired,
                Property.asEnumMap(subtitleData));
    }

    public URI uploadSubtitles(BaseInfo baseInfo, SubFile... subtitles)
            throws XmlRpcFault {
    ...
    public List<String> detectLanguage(byte[] data) throws XmlRpcFault {
        ...
        Map<String, Map<String, String>> response = 
                (Map<String, Map<String, String>>) invoke("DetectLanguage",
                token, singleton(parameter));
    ...
    @SuppressWarnings("unchecked")
    public Map<String, Integer> checkSubHash(Collection<String> hashes)
            throws XmlRpcFault {
        ...
        for (Entry<String, ?> entry : subHashData.entrySet()) {
            // non-existing subtitles are represented
            // as Integer 0, not String "0"
            subHashMap.put(entry.getKey(), 
    ...
    @SuppressWarnings("unchecked")
    public Map<String, MovieDescriptor> checkMovieHash(
            Collection<String> hashes) throws XmlRpcFault {
        Map<?, ?> response = invoke("CheckMovieHash", token, hashes);

        Map<String, ?> movieHashData = (Map<String, ?>) response.get("data");
        Map<String, MovieDescriptor> movieHashMap =
                new HashMap<String, MovieDescriptor>();
     ...
     *
     * @param status status code and message (e.g. 200 OK,
     * 401 Unauthorized, ...)
    ...
    public static final class Query extends HashMap<String, Object> {
        ...
        private Query(String moviehash, String moviebytesize,
        ...
        public static Query forHash(String moviehash, long moviebytesize,
                String... sublanguageids) {
            return new Query(moviehash, Long.toString(moviebytesize),
                    sublanguageids);
        }
    ...
    public static final class TryUploadResponse {
        ...
        private TryUploadResponse(boolean uploadRequired,
                Map<Property, String> subtitleData) {
