...
/**
 * Hash code is based on Media Player Classic. In natural 
 * language it calculates: size + 64bit checksum of the first
 * and last 64k (even if they overlap because the file is smaller than
 * 128k).
 */
public final class OpenSubtitlesHasher {
    ...
    public static String computeHash(InputStream stream, long length)
        ...
        // chunks will overlap if length is smaller than two chunks
        byte[] chunkBytes =
                new byte[(int) Math.min(2 * HASH_CHUNK_SIZE, length)];
        ...
        // seek to position of the tail chunk, or not at 
        //all if length is smaller than two chunks
        while (position < tailChunkPosition &&
                (position += in.skip(tailChunkPosition - position)) >= 0);

        // second chunk, or the rest of the data if 
        //length is smaller than two chunks
        in.readFully(chunkBytes, chunkSizeForFile,
                chunkBytes.length - chunkSizeForFile);
