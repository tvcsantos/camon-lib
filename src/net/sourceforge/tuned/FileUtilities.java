...
public final class FileUtilities {
     ...
     */
    public static final Pattern EXTENSION =
    ...
    public static boolean containsOnly(Iterable<File> files,
            FileFilter filter) {
        for (File file : files) {
    ...
    public static List<File> filter(Iterable<File> files,
            FileFilter... filters) {
    ...
    public static List<File> flatten(Iterable<File> roots,
            int maxDepth) {
    ...
    private static void listFiles(File folder, int depth,
            List<File> files, int maxDepth) {
        if (depth > maxDepth) {
            return;
        }
    ...
    public static File validateFileName(File file) {
        // windows drives (e.g. c:, d:, etc.) are never invalid
        // because name will be an empty string
    ...
    public static File validateFilePath(File path) {
        ...
        // initialize with root node, keep original root object
        // if possible (so we don't loose the drive on windows)
        File validatedPath = validateFileName(nodes.next());

        // validate the rest of the path
        while (nodes.hasNext()) {
            validatedPath = new File(validatedPath,
                    validateFileName(nodes.next().getName()));
        }
