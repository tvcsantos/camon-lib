...
public abstract class Timer implements Runnable {
    ...
    private final ThreadFactory threadFactory =
            new DefaultThreadFactory("Timer", Thread.NORM_PRIORITY, true);
    private ScheduledThreadPoolExecutor executor;
    private ScheduledFuture<?> scheduledFuture;
    private Thread shutdownHook;

    public synchronized void set(long delay, TimeUnit unit,
            boolean runBeforeShutdown) {
        ...
        if (shutdownHook != null) {
            try {
                if (shutdownHook != Thread.currentThread()) {
                    // can't remove shutdown hooks anymore,
                    // once runtime is shutting down,
                    // so don't remove the shutdown hook,
                    // if we are running on the shutdown hook
