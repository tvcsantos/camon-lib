...
public class PersonListExpander extends AbstractExpander implements Expander {
    ...
    public void expand(BibtexFile file) throws ExpansionException {
        for (Iterator<BibtexAbstractEntry> entryIt =
                file.getEntries().iterator(); entryIt.hasNext();) {
            BibtexAbstractEntry next = entryIt.next();
            ...
            if (expandAuthors && entry.getFieldValue("author") != null) {
                if (entry.getFieldValue("author") instanceof BibtexString) {
                    try {
                        entry.setField("author", BibtexPersonListParser.parse(
                                (BibtexString) entry.getFieldValue("author"),
                                    entry.getEntryKey()));
            ...
            if (expandEditors && entry.getFieldValue("editor") != null) {
                if (entry.getFieldValue("editor") instanceof BibtexString) {
                    try {
                        entry.setField("editor", BibtexPersonListParser.parse(
                                (BibtexString) entry.getFieldValue("editor"),
                                    entry.getEntryKey()));
                    } catch (PersonListParserException e) {
                        throwExpansionException(e);
                    }
